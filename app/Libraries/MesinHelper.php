...
class MesinHelper
{
    ...
    public function checkDuplicate($name)
    {
        ...
        $similar_items = $this->db->table('mesin_cuci')
            ->select('nama_mesin')
            ->get()
            ->getResult();

        foreach ($similar_items as $item) {
            $similarity = $this->advancedSimilarity($normalized_name, $item->nama_mesin);
    ...
    public function validateMesinData($data)
    {
        $rules = [
            'nama_mesin' => [
                ...
                'errors' => [
                    ...
                    'min_length' => 'Nama Mesin minimal 3 karakter.',
                    'max_length' => 'Nama Mesin maksimal 100 karakter.'
                ]
            ],
        ];
