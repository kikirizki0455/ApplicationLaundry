                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($p['status'] === 'pending'): ?>
                                    <form action="<?= site_url('pengelolaan/pengeringan/start/' . $p['id_pengeringan']); ?>" method="post" style="display:inline;">
                                        <?= csrf_field(); ?>
                                        <button type="submit" class="btn btn-success btn-sm">Mulai</button>
                                    </form>
                                <?php elseif ($p['status'] === 'in_progress'): ?>
                                    <form action="<?= site_url('pengelolaan/pengeringan/StatMove/' . $p['id_pengeringan']); ?>" method="post" style="display:inline;">
                                        <?= csrf_field(); ?>
                                        <button type="submit" class="btn btn-warning btn-sm">Selesai</button>
                                    </form>
                                <?php elseif ($p['status'] === 'ready_move'): ?>
                                    <a href="<?= site_url('pengelolaan/pengeringan/move/' . $p['id_pengeringan']); ?>" class="btn btn-primary btn-sm">pindahkan</a>
                                <?php endif; ?>
                            </td>
